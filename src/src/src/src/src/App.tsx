...
function Dashboard({ userId, email }:{ userId:string; email:string }){
  ...
  useEffect(()=>{(async()=>{
    ...
    if(!e){
      const empty:Entreprise={ user_id:userId, nom:'Pompes Funèbres Loubet-Victor', adresse:'', telephone:'', email: email||'', siret:'', signature_data_url:null }
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16}}>
            ...
            <div>
              {current ? (
                ...
                <div style={{border:'1px dashed #bbb',borderRadius:8,padding:24,textAlign:'center',color:'#666'}}>Sélectionnez un dossier</div>
              )}
            </div>
          </div>
        )}
